...
Добавил в скрипт Perceptron метод OnCollisionEnter, который определяет цвета столкнувшихся объектов, по которым расчитывает результирующий цвет объекта `красный — 0 — falce`, `зелёный — 1 — true`
```cs
...
Операцию XOR не визуализировал, т.к. Перцептрон не может научиться её выполнению.
